...
$graphData['labels']=['Avtal','Kontant'];
?>
<?php if ($search_ajax != 'yes'): ?>
  <div class="whitebgDiv">
    <?php endif; ?>
    <?php if ($search_ajax != 'yes'  && $canvasId=='kontant_vs_contract_total'): ?>
      <div class="form-box">
    ...
    <div class="ajax-load-lc-data">
        <div class="row">
				...
				<div class="table-responsive">
					<table id="table_lc_larms" class="parent-table table table-condensed table-striped dataTable no-footer" cellspacing="0" width="100%" aria-describedby="table_info" role="grid" style="width: 100%;">
						...
						<tbody>
							...
							<tr>
								<td>Totalt</td>
								<td><?php echo $contracts['overall_total']; ?></td>
								<td><?php echo $kontants['overall_total']; ?></td>
								<td><?php echo $contracts['overall_total'] + $kontants['overall_total']; ?></td>

							</tr>
							<tr>
								<td>Procent</td>
								<td><?php echo $contactPercent=($contracts['overall_total'] > 0) ? round(($contracts['overall_total'] / ($contracts['overall_total'] + $kontants['overall_total']) * 100)) : 0; ?>%</td>
								<td><?php echo $kontantPercent=($kontants['overall_total'] > 0) ? round(($kontants['overall_total'] / ($contracts['overall_total'] + $kontants['overall_total']) * 100)) : 0; ?>%</td>
								<td></td>
									<?php $graphData['datasets'][0]['data'] = [$contactPercent,$kontantPercent]; ?>
							</tr>
							</table>
						  </td>
						  </tr>
							...
							<td class="tdpaddding0" colspan="4">
							  ...
							  <tr>
								  <td><?php echo $year; ?></td>
								  <td><?php echo $contractThisYear = isset($contracts[$year]) ? $contracts[$year] : 0; ?></td>
								  <td><?php echo $kontantsThisyear = isset($kontants[$year]) ? $kontants[$year] : 0; ?></td>
								  <td><?php echo $totalThisYear = (isset($contracts[$year]) ? $contracts[$year] : 0) + (isset($kontants[$year]) ? $kontants[$year] : 0); ?></td>

							  </tr>
							  <tr>
								  <td>Procent</td>
								  <td><?php echo ($contractThisYear > 0) ? round($contractThisYear / $totalThisYear * 100) : 0; ?>%</td>
								  <td><?php echo ($kontantsThisyear > 0) ? round($kontantsThisyear / $totalThisYear * 100) : 0; ?>%</td>
								  <td></td>
							  </tr>
							  </table>
						  </td>
            ...
            <div class="col-md-6 col-xs-12">
                ...
                <div class="chart-container chart-container-only-pie" style="width:400px;">
                    <canvas id="lc_grpah_<?php echo $canvasId; ?>" width="100px" height="100px" class="pie_chart-monitor"></canvas>
                </div>
